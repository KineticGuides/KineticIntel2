<?php              
   ...
   $wc=0;
   foreach($d as $g) {
    $sql="select id, the_date from kv_dates where the_date = '" . $g['r'] . "'";
    $hh=$X->sql($sql);
    if (sizeof($hh)>0) {
    $end_date = $g['r'];
    $sql="select id, the_date from kv_dates where id = " . $hh[0]['id']-90;
    $hh=$X->sql($sql);
    if (sizeof($hh)>0) {
        $start_date=$hh[0]['the_date'];
        $list=array();
        $list['low_date']=$start_date;
        $list['high_date']=$end_date;
...
$sql="select id from ksa_security_snapshot where security_id = " . $security_id . " and high_date = '" . $high_date . "' and snapshot_type = '" . $snapshot_type . "'";
$rrr=$X->sql($sql);
